...
for q in range(1000):
    ...
    population=sorted(population,key=getScore,reverse=True)
    #hybrid
    for  i in range(50*popuSize//100,70*popuSize//100):
        g1=population[randint(0,30*popuSize//100)]
        g2=population[randint(0,30*popuSize//100)]
        population[i]=hybrid(g1,g2)
    #completly new
    for i in range(80*popuSize//100,100*popuSize//100-1):
        population[i]=gene()
    for  i in range(70*popuSize//100,80*popuSize//100):
        g1=population[randint(2,30*popuSize//100)]
        g2=population[randint(80*popuSize//100,100*popuSize//100-1)]
        population[i]=hybrid(g1,g2)
    #mutate, 20%
    for i in range(20*popuSize//100):
        cur=population[randint(3,80*popuSize//100)]
        for layer in range(len(struct)):
            for unit in range(struct[layer]):
                for weight in range(cur.weight[layer][unit].size):
                    if randint(0, 9)< 3:
                        cur.weight[layer][unit][weight] = uniform(-0.1,0.1)*cur.weight[layer][unit][weight]
                if randint(0, 9) == 3:
                    cur.bias[layer][unit] = uniform(-0.1,0.1)*cur.bias[layer][unit]
    for i in range(3):
        cur= population[randint(80 * popuSize // 100, 100 * popuSize // 100-1)]
        for layer in range(len(struct)):
            for unit in range(struct[layer]):
                for weight in range(cur.weight[layer][unit].size):
                    cur.weight[layer][unit][weight] = population[0].weight[layer][unit][weight]
                cur.bias[layer][unit] = population[0].bias[layer][unit]
    record(population,gen=q+1+119)
    print('time ',time()-sta)
